...
import _init_paths
from dataset import small_robotcar

from utils.logger import setup_logger
from my_model import loftr_model
...
def parser_args():
    ...
    parser.add_argument('--dataname', help='dataname', default='robotcar', choices=['robotcar'])
...
def main_worker(args, logger):
    ...
    # build model
    model = loftr_model.LOFTR()
    model = model.cuda()
    ema_m = ModelEma(model, args.ema_decay) # 0.9997
    dic=torch.load('/home2/wangyj/mvit/model_best.pth.tar')["state_dict"]
    print('yesy______________________________________________________')
    new_state_dict = OrderedDict()
    for k, v in dic.items():
        myname = k[7:] 
        new_state_dict[myname] = v 
    ...
    # Data loading code
    train_dataset = small_robotcar.LIGHT_DIS(True)
    val_dataset=small_robotcar.LIGHT_DIS(False)
...
def train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger):
    ...
    for i, meta in enumerate(train_loader):
        ...
        speed_gpu.update(image0.size(0) / batch_time.val, batch_time.val)
        speed_all.update(image0.size(0) * dist.get_world_size() / batch_time.val, batch_time.val)
